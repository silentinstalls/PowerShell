...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/a/AntibodySoftware/WizTree"
...
Write-Host "Latest WizTree version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\WizTree-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\WizTree-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\WizTree-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "WizTree installation completed."
