...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/Greenshot/Greenshot"
...
Write-Host "Latest Greenshot version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Greenshot-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Greenshot-latest.exe" -ArgumentList '/SP- /SUPPRESSMSGBOXES /VERYSILENT /NORESTART' 

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Greenshot-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Greenshot installation completed."
