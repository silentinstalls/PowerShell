...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/r/RealVNC/VNCServer"
...
Write-Host "Latest VNCServer version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\VNCServer-latest.zip")
Expand-Archive -Path "$env:TEMP\VNCServer-latest.zip" -DestinationPath "$env:TEMP\VNCServer" -Force

# Start the install process.
Set-Location -Path $env:TEMP\VNCServer
msiexec.exe /i $((Get-ChildItem -Path $env:TEMP\VNCServer -Filter *64bit*).Name) /qn

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\VNCServer-latest.zip" -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$env:TEMP\VNCServer" -Recurse  -Force -ErrorAction SilentlyContinue

Write-Host "VNCServer installation completed."
