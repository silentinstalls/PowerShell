...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/r/RevoUninstaller/RevoUninstaller"
...
Write-Host "Latest RevoUninstaller version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\RevoUninstaller-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\RevoUninstaller-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\RevoUninstaller-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "RevoUninstaller installation completed."
