...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/f/FastStone/Viewer"
...
Write-Host "Latest FastStone Viewer version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\FastStoneViewer-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\FastStoneViewer-latest.exe" -ArgumentList '/S /NCRC' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\FastStoneViewer-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "FastStone Viewer installation completed."
