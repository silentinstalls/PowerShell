...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/f/FastStone/Capture"
...
Write-Host "Latest FastStone Capture version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\FastStoneCapture-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\FastStoneCapture-latest.exe" -ArgumentList '/S /NCRC' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\FastStoneCapture-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "FastStone Capture installation completed."
