...
Write-Host "Zoom version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Zoom-latest.msi")

# Start the install process.
Set-Location -Path $env:TEMP
Start-Process -FilePath msiexec.exe -ArgumentList '/i Zoom-latest.msi /qn' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Zoom-latest.msi" -Force -ErrorAction SilentlyContinue
