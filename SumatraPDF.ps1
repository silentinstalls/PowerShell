...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/s/SumatraPDF/SumatraPDF"
...
Write-Host "Latest SumatraPDF version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\SumatraPDF-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\SumatraPDF-latest.exe" -ArgumentList '-install -s' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\SumatraPDF-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "SumatraPDF installation completed."
