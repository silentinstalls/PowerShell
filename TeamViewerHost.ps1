...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/t/TeamViewer/TeamViewer/Host"
...
Write-Host "Latest TeamViewer Host version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\TeamViewerHost-latest.zip")
Expand-Archive -Path "$env:TEMP\TeamViewerHost-latest.zip" -DestinationPath "$env:TEMP\TeamViewerHost" -Force

# Start the install process.
Set-Location -Path $env:TEMP\TeamViewerHost
Start-Process -FilePath msiexec.exe -ArgumentList '/i update.msi /qn' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\TeamViewerHost-latest.zip" -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$env:TEMP\TeamViewerHost" -Recurse  -Force -ErrorAction SilentlyContinue

Write-Host "TeamViewer Host installation completed."
