...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/d/Dropbox/Dropbox"
...
Write-Host "Latest Dropbox version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Dropbox-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Dropbox-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Dropbox-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Dropbox installation completed."
