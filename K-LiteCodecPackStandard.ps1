...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CodecGuide/K-LiteCodecPack/Standard"
...
Write-Host "K-Lite Codec Pack Standard version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\K-LiteCodecPackStandard-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\K-LiteCodecPackStandard-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\K-LiteCodecPackStandard-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "K-Lite Codec Pack Standard installation completed."
