...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CodecGuide/K-LiteCodecPack/Mega"
...
Write-Host "K-Lite Codec Pack Mega version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\K-LiteCodecPackMega-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\K-LiteCodecPackMega-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\K-LiteCodecPackMega-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "K-Lite Codec Pack Mega installation completed."
