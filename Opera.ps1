...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/o/Opera/Opera"
...
Write-Host "Latest Opera version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Opera-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Opera-latest.exe" -ArgumentList '/install /silent /launchopera=0 /setdefaultbrowser=0 /allusers=1' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Opera-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Opera installation completed."
