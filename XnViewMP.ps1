...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/x/XnSoft/XnViewMP"
...
Write-Host "Latest XnViewMP version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\XnViewMP-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\XnViewMP-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\XnViewMP-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "XnViewMP installation completed."
