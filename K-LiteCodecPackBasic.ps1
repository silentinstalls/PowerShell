...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CodecGuide/K-LiteCodecPack/Basic"
...
Write-Host "K-Lite Codec Pack Basic version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\K-LiteCodecPackBasic-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\K-LiteCodecPackBasic-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\K-LiteCodecPackBasic-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "K-Lite Codec Pack Basic installation completed."
