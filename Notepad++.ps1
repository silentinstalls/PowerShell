...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/n/Notepad%2B%2B/Notepad%2B%2B"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Notepad++-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Notepad++-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Notepad++-latest.exe" -Force -ErrorAction SilentlyContinue
