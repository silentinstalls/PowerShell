...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CubeSoft/CubePDF"
...
Write-Host "Latest CubePDF version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\CubePDF-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\CubePDF-latest.exe" -ArgumentList '/SP- /VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\CubePDF-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "CubePDF installation completed."
