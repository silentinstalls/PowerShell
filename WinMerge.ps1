...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/w/WinMerge/WinMerge"
...
Write-Host "Latest WinMerge version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\WinMerge-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\WinMerge-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\WinMerge-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "WinMerge installation completed."
