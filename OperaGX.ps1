...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/o/Opera/OperaGX"
...
Write-Host "Latest OperaGX version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\OperaGX-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\OperaGX-latest.exe" -ArgumentList '/install /silent /launchopera=0 /setdefaultbrowser=0 /allusers=1' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\OperaGX-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "OperaGX installation completed."
