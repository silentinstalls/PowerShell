...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/i/IPVanish/SugarSync"
...
Write-Host "Latest SugarSync version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\SugarSync-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\SugarSync-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\SugarSync-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "SugarSync installation completed."
