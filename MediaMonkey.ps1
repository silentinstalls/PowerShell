...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/v/VentisMedia/MediaMonkey/2024"
...
Write-Host "Latest MediaMonkey version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\MediaMonkey-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\MediaMonkey-latest.exe" -ArgumentList '/SP- /VERYSILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\MediaMonkey-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "MediaMonkey installation completed."
