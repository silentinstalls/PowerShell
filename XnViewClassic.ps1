...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/x/XnSoft/XnView/Classic"
...
Write-Host "Latest XnView Classic version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\XnViewClassic-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\XnViewClassic-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\XnViewClassic-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "XnView Classic installation completed."
