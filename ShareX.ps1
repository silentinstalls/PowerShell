...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/s/ShareX/ShareX"
...
Write-Host "Latest ShareX version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\ShareX-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\ShareX-latest.exe" -ArgumentList '/SP- /SUPPRESSMSGBOXES /VERYSILENT /NORESTART' 

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\ShareX-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "ShareX installation completed."
