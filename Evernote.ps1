...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/e/Evernote/Evernote"
...
Write-Host "Latest Evernote version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Evernote-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Evernote-latest.exe" -ArgumentList '/AllUsers /S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Evernote-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Evernote installation completed."
