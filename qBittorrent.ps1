...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/q/qBittorrent/qBittorrent"
...
Write-Host "Latest qBittorrent version: $latestVersion."
...
Invoke-WebRequest -UserAgent "Wget" -Uri $installerUrl -OutFile "$env:TEMP\qBittorrent-latest.exe" 

# Start the install process.
Start-Process -FilePath "$env:TEMP\qBittorrent-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\qBittorrent-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "qBittorrent installation completed."
