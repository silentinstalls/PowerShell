...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/m/Microsoft/VCRedist/2015%2B/x64"
...
Write-Host "Latest VCRedist 2015+ version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\VCRedist.2015+-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\VCRedist.2015+-latest.exe" -ArgumentList '/install /quiet /norestart' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\VCRedist.2015+-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "VCRedist 2015+ installation completed."
