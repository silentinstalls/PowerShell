...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/GIMP/GIMP/3"
...
Write-Host "Latest GIMP version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\GIMP-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\GIMP-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART /ALLUSERS' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\GIMP-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "GIMP installation completed."
