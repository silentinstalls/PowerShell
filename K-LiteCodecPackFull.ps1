...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CodecGuide/K-LiteCodecPack/Full"
...
Write-Host "K-Lite Codec Pack Full version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\K-LiteCodecPackFull-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\K-LiteCodecPackFull-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\K-LiteCodecPackFull-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "K-Lite Codec Pack Full installation completed."
