...
Write-Host "Latest PuTTY version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\PuTTY-latest.msi")

# Start the install process.
Set-Location -Path $env:TEMP
Start-Process -FilePath msiexec.exe -ArgumentList '/i PuTTY-latest.msi /qn' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\PuTTY-latest.msi" -Force -ErrorAction SilentlyContinue

Write-Host "PuTTY installation completed."
