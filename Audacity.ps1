...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/a/Audacity/Audacity"
...
Write-Host "Audacity version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Audacity-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Audacity-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Audacity-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Audacity installation completed."
