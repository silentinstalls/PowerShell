...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/c/CodeSector/TeraCopy"
...
Write-Host "Latest TeraCopy version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\TeraCopy-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\TeraCopy-latest.exe" -ArgumentList '/exenoupdates /exenoui /quiet /norestart' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\TeraCopy-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "TeraCopy installation completed."
