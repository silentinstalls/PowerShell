...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/a/Apple/iTunes"
...
Write-Host "iTunes version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\iTunes-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\iTunes-latest.exe" -ArgumentList '/quiet /norestart' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\iTunes-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "iTunes installation completed."
