...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/h/HandBrake/HandBrake"
...
Write-Host "Latest HandBrake version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\HandBrake-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\HandBrake-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\HandBrake-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "HandBrake installation completed."
