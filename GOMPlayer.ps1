...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/GOMLab/GOMPlayer"
...
Write-Host "GOM Player version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\GOMPlayer-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\GOMPlayer-latest.exe" -ArgumentList '/S' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\GOMPlayer-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "GOM Player installation completed."
