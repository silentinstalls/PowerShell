...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/Google/EarthPro"
...
Write-Host "Latest Google Earth version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\GoogleEarth-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\GoogleEarth-latest.exe" -ArgumentList 'OMAHA=1' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\GoogleEarth-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "GoogleEarth installation completed."
