...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/Google/GoogleDrive"
...
Write-Host "Latest Google Drive version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\GoogleDrive-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\GoogleDrive-latest.exe" -ArgumentList '--silent --skip_launch_new --gsuite_shortcuts=false' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\GoogleDrive-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Google Drive installation completed."
