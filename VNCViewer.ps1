...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/r/RealVNC/VNCViewer"
...
Write-Host "Latest VNCViewer version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\VNCViewer-latest.zip")
Expand-Archive -Path "$env:TEMP\VNCViewer-latest.zip" -DestinationPath "$env:TEMP\VNCViewer" -Force

# Start the install process.
Set-Location -Path $env:TEMP\VNCViewer
msiexec.exe /i $((Get-ChildItem -Path $env:TEMP\VNCViewer -Filter *64bit*).Name) /qn

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\VNCViewer-latest.zip" -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$env:TEMP\VNCViewer" -Recurse  -Force -ErrorAction SilentlyContinue

Write-Host "VNCViewer installation completed."
