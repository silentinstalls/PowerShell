...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/m/Mozilla/Thunderbird"
...
Write-Host "Thunderbird version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Thunderbird-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Thunderbird-latest.exe" -ArgumentList '/S /PreventRebootRequired=true' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Thunderbird-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Thunderbird installation completed."
