...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/g/Giorgiotani/Peazip"
...
Write-Host "Peazip version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Peazip-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\Peazip-latest.exe" -ArgumentList '/SP- /SILENT /SUPPRESSMSGBOXES /NORESTART' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Peazip-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "Peazip installation completed."
