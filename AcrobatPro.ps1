...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/a/Adobe/Acrobat/Pro"
...
Write-Host "Latest Adobe Acrobat Pro version: $latestVersion."
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\Acrobat.Pro-latest.zip")
Expand-Archive -Path "$env:TEMP\Acrobat.Pro-latest.zip" -DestinationPath "$env:TEMP\Acrobat.Pro" -Force

# Start the install process.
Start-Process -FilePath "$env:TEMP\Acrobat.Pro\Adobe Acrobat\setup.exe" -ArgumentList '/sAll /rs /msi' -Wait


# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\Acrobat.Pro-latest.zip" -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$env:TEMP\Acrobat.Pro" -Recurse  -Force -ErrorAction SilentlyContinue

Write-Host "Adobe Acrobat Pro installation completed."
