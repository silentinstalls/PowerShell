...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/n/NVAccess/NVDA"
...
Write-Host "NVAccess version: $latestVersion"
...
$webClient.DownloadFile($installerUrl, "$env:TEMP\NVAccess-latest.exe")

# Start the install process.
Start-Process -FilePath "$env:TEMP\NVAccess-latest.exe" -ArgumentList '--install-silent' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\NVAccess-latest.exe" -Force -ErrorAction SilentlyContinue

Write-Host "NVAccess installation completed."
