...
$apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/d/DominikReichl/KeePass"
...
Write-Host "KeePass version: $latestVersion"
...
Invoke-WebRequest -UserAgent 'Wget' -Uri $installerUrl -OutFile "$env:TEMP\KeePass.msi"


# Start the install process.
Set-Location -Path $env:TEMP
Start-Process -FilePath msiexec.exe -ArgumentList '/i KeePass.msi /qn' -Wait

# Delete the downloaded installer file.
Remove-Item -Path "$env:TEMP\KeePass.msi" -Force -ErrorAction SilentlyContinue

Write-Host "KeePass installation completed."
